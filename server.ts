...
import { User, DB_User, DB_Room, Socket_RoomJoined, Socket_RoomMemberLeft, Socket_RoomMemberVoted, Socket_RoomMemberUpdated, Socket_RoomResetti, ResetRoom, ChangeDeck, Socket_RoomDeckChange } from './types'
  ...
  socket.on('join-room-no-confirmation', (new_user: User) => {
    if (process.env.NODE_ENV === 'development') {
      ...
      if (existing_room.length > 0) {
        ...
        if (memberIndex !== -1) {
          existing_room[0].members[memberIndex] = { ...existing_room[0].members[memberIndex], ...user }
        } else {
          existing_room[0].members.push(user)
        }
